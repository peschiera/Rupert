...
int S1, S2, S3, S4, S5;
boolean flag = 0;
int KP = 10; //valores que ayustan la velocidad dependiendo el error.
int KD = 5;
int error_ant = 0;

//Espectro de Lectura de Sensores
int Umbral = 40;
int UmbralS2 = 40; //la lectura mas chica de este sensor es 65

//Velocidades motores (maximo valor posible 255)
int Vmed = 150; //Velocidad medio
...
int Sensores() {
  ...
  //Serial.print(S1);     //Si esta sobre negro vale "1" si esta sobre blanco "0"
...
int Error(int lectura) {
  ...
  switch (lectura) {
    ...
    case 1111://lateral izq
      ...
      break;
    default:
      //error=6; // Ver para agregar algo en el caso default luego
      break;
  }
...
void moverMotores(int error) {
  ...
  switch (error) {
    case -4:
      ...
      digitalWrite(pin_DIRM2, 0);
      analogWrite(pin_PWMM1, constrain(Vmed + Pwm, 0, 255));
      analogWrite(pin_PWMM2, constrain(Vmed - Pwm, 0, 255));
      break;
    case 4:
      digitalWrite(pin_DIRM1, 0);
    ...
    default:
      ...
      analogWrite(pin_PWMM1, constrain(Vmed + Pwm, 0, 255));
      analogWrite(pin_PWMM2, constrain(Vmed - Pwm, 0, 255));
      delay(100); // Probar si este delay es el indicado para que funcione bien
      break;
  }
